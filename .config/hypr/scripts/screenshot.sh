...
# Saves to ~/Pictures by default.
# Editor defaults to Satty but can be changed via --editor=<name> or SCREENSHOT_EDITOR env.
...
# Toggle off if slurp is already running
pkill slurp && exit 0
...
smart | *)
  ...
  # If selection area < 20px², snap to the window/monitor containing the click point
...
(
  ACTION=$(notify-send "Screenshot saved to clipboard and file" "Edit with Print (or click this)" \
